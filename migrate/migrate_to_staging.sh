...
curl -X POST http://localhost:3002/api/server-exec \
  -H "Content-Type: application/json" \
  -d '{
    "command": "mysql -u root -p***REMOVIDO*** -e \"CREATE DATABASE IF NOT EXISTS staging_visualdesign; CREATE DATABASE IF NOT EXISTS staging_sites;\" && echo \"Bancos staging criados\"",
    "server": "contabo"
  }' 2>/dev/null
    ...
    if [ ! -z "$site_path" ]; then
        ...
        if [ ! -z "$db_name" ]; then
            ...
            curl -X POST http://localhost:3002/api/server-exec \
              -H "Content-Type: application/json" \
              -d "{
                \"command\": \"gunzip < /tmp/${site_name}_db_${DATE}.sql.gz | mysql -u root -p***REMOVIDO*** staging_${site_name} && echo 'Banco importado'\",
                \"server\": \"contabo\"
              }" 2>/dev/null
