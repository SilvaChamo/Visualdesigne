...
function RenovacaoStatusContent({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params)
  ...
  const load = useCallback(async () => {
    try {
      const res = await fetch(`/api/renewals/pagamento/${id}`)
  ...
  const handleUpload = async (file: File) => {
    ...
    try {
      ...
      const res = await fetch(`/api/renewals/pagamento/${id}/comprovativo`, { method: 'POST', body: form })
      const data = await res.json()
  ...
  const handlePayStripe = async () => {
    ...
    setError('')
    try {
      const res = await fetch(`/api/renewals/pagamento/${id}/stripe-session`, { method: 'POST' })
      const data = await res.json()
  ...
  const steps: { key: string; label: string; done: boolean; current: boolean }[] = [
    { key: 'criado', label: 'Factura Criada', done: true, current: false },
    {
      key: 'aguarda',
      label: pedido.status === 'rejected' ? 'Rejeitado' : pedido.status === 'confirmed' ? 'Pago' : 'Aguardando Pagamento',
      done: pedido.status !== 'pending',
      current: pedido.status === 'pending',
    },
    { key: 'confirmado', label: 'Serviço Renovado', done: pedido.status === 'confirmed', current: false },
  ]
          ...
          <div className="flex items-start justify-between gap-3 mb-6">
            <div>
              <p className="text-xs font-bold uppercase tracking-wide text-slate-400 dark:text-zinc-500">
                Renovação de {pedido.renewal_type === 'domain' ? 'Domínio' : 'Hospedagem'}
              </p>
              <h1 className="text-lg font-bold text-slate-900 dark:text-white mt-1 truncate">{pedido.service_name}</h1>
              <p className="text-2xl font-bold text-slate-900 dark:text-white mt-1">MT {formatMt(pedido.valor_mt)}</p>
            </div>
            <span
...
export default function RenovacaoStatusPage({ params }: { params: Promise<{ id: string }> }) {
  return (
    <Suspense
      fallback={
        <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-zinc-950">
    ...
    >
      <RenovacaoStatusContent params={params} />
    </Suspense>
  )
}
