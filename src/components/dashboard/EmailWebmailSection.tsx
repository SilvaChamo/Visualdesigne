...
export function EmailWebmailSection({
      ...
      try {
        ...
        if (data.success && data.emails && data.emails.length > 0) {
          setEmailsOrigem(data.emails.map((e: any) => ({
            ...
            password: passwordMap[e.email] || (e.email.endsWith('@visualdesigne.com') ? '***REMOVIDO***' : '')
          })))
        } else {
          ...
          try {
            ...
            if (data3.success && data3.contas?.length > 0) {
              setEmailsOrigem(data3.contas.map((c: any) => ({
                ...
                password: passwordMap[c.email] || (c.email.endsWith('@visualdesigne.com') ? '***REMOVIDO***' : '')
              })))
            }
          } catch { }
        }
  ...
  useEffect(() => {
    ...
    const senhaImap = emailOrigemPassword || contaActual?.password || (emailOrigem?.endsWith('@visualdesigne.com') ? '***REMOVIDO***' : '')
